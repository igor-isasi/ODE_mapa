...
class Mapa:
    ...
    def generarMapa(self, filtros, añosInd, colormapInd, fechaIncidencia):
        ...
        errorApiInd = self.__generarIndicadoresColormap(mapa, filtros, añosInd, colormapInd, geojson_fields, geojson_aliases)
        # Trafico
        errorApiTraf = self.__generarEventosTrafico(mapa, filtros, fechaIncidencia)
        # Eventos
        errorApiEv = self.__generarEventosGenerales(mapa, filtros)
        # Eventos administrativos
        errorApiEnt = self.__generarEntidades(mapa, filtros)
    ...
    def __generarEntidades(self, mapa, filtros):
        errorApi = None
    ...
    def __añadirEventos(self, jsonEventos, fg):
        ...
        for evento in jsonEventos['items']:
            if self.__esEuskadi(evento['provinceNoraCode']):
                if 'priceEs' in evento and self.__tieneHoraAsignada(evento):
                ...
                elif 'priceEs' in evento and not self.__tieneHoraAsignada(evento):
                    myIFrame = folium.IFrame('<strong>Nombre:</strong> ' + evento['nameEs'] + 
                                            '<br><br><strong>Tipo de evento:</strong> ' + evento['typeEs'] + 
                                            '<br><br><strong>Día de inicio:</strong> ' + evento['startDate'].split("T")[0] +
                                            '<br><br><strong>Hora de inicio:</strong> Sin información' + 
                                            '<br><br><strong>Día de fin:</strong> ' + evento['endDate'].split("T")[0] +
                                            '<br><br><strong>Hora de fin:</strong> Sin información' +
                                            '<br><br><strong>Precio:</strong> ' + evento['priceEs'])
                elif 'priceEs' not in evento and self.__tieneHoraAsignada(evento):
                    myIFrame = folium.IFrame('<strong>Nombre:</strong> ' + evento['nameEs'] + 
                                            '<br><br><strong>Tipo de evento:</strong> ' + evento['typeEs'] + 
                                            '<br><br><strong>Día de inicio:</strong> ' + evento['startDate'].split("T")[0] +
                                            '<br><br><strong>Hora de inicio:</strong> ' + evento['startDate'].split("T")[1][:-1] + 
    ...
    def __añadirEntidades(self, jsonEntidades, fg):
        myIFrame = None
        myIcon = None
        myPopup = None
        for entidad in jsonEntidades:
            ...
            myPopup = folium.Popup(myIFrame, min_width=300, max_width=500)
            #myIcon = self.__getIconoEvento(evento)
            lat, long = self.__convertirCoords(entidad['geoPosition']['position2D']['x'], entidad['geoPosition']['position2D']['y'])
            folium.Marker(location=[lat, long], popup = myPopup, icon = myIcon).add_to(fg)
    
    def __tieneHoraAsignada(self, evento):
    ...
    def __calcularRankingInd(self, indicator, jsonInd):
        rankingInd = []
    ...
    def __convertirCoords(self, utm_x, utm_y):
        huso = 30
        srcProj = pyproj.Proj(proj="utm", zone=huso, ellps="WGS84")
        dstProj = pyproj.Proj(proj="longlat")
